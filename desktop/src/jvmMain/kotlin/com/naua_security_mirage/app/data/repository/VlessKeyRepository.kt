...
class VlessKeyRepository(
    ...
    private fun deriveKey(): ByteArray {
        ...
        for (i in 0 until 32) {
            derived[i] = (BASE_SEED[i].toInt() xor CERT_DIGEST[i].toInt()).toByte()
        }
        return derived
    }
    ...
    private val apiUrl: String by lazy {
        System.getenv("MIRAGE_API_URL") ?: mask(ENC_API_URL)
    }
    private val defaultUuid: String by lazy {
        System.getenv("MIRAGE_DEFAULT_UUID") ?: mask(ENC_DEFAULT_UUID)
    }
    private val defaultPbk: String by lazy {
        System.getenv("MIRAGE_DEFAULT_PBK") ?: mask(ENC_DEFAULT_PBK)
    }
    private val defaultSni: String by lazy {
        System.getenv("MIRAGE_DEFAULT_SNI") ?: mask(ENC_DEFAULT_SNI)
    }
    private val fbHost1: String by lazy {
        System.getenv("MIRAGE_FB_HOST_1") ?: mask(ENC_FB_HOST_1)
    }
    private val fbHost2: String by lazy {
        System.getenv("MIRAGE_FB_HOST_2") ?: mask(ENC_FB_HOST_2)
    }
    private val fbHost3: String by lazy {
        System.getenv("MIRAGE_FB_HOST_3") ?: mask(ENC_FB_HOST_3)
    }
        ...
        kotlinx.coroutines.withTimeoutOrNull(4000) {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    AppLogger.w(TAG, "API returned code ${response.code}, falling back to static servers")
                }
            } catch (e: Exception) {
                AppLogger.e(TAG, "Failed to fetch servers from API: ${e.message}, using fallback servers")
            }
            Unit
        }

        // Ensure we always have fallback servers
    ...
    companion object {
        ...
        private val CERT_DIGEST = byteArrayOf(
            0xA0.toByte(), 0x83.toByte(), 0x30.toByte(), 0x71.toByte(), 0xE8.toByte(), 0x50.toByte(), 0xA9.toByte(), 0xBC.toByte(),
            0xAA.toByte(), 0x04.toByte(), 0x54.toByte(), 0x14.toByte(), 0xC0.toByte(), 0x5B.toByte(), 0xFE.toByte(), 0xA2.toByte(),
            0x2F.toByte(), 0xEE.toByte(), 0xB0.toByte(), 0xF9.toByte(), 0xB1.toByte(), 0x29.toByte(), 0x39.toByte(), 0xA1.toByte(),
            0x22.toByte(), 0xC7.toByte(), 0xC1.toByte(), 0x90.toByte(), 0x27.toByte(), 0x59.toByte(), 0xB5.toByte(), 0x85.toByte()
        )
